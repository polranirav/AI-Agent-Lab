...
def start_run(session_id: Optional[str], orchestrator: str,
    ...
    """Create a run record (status 'running') and return its UUID."""
    run_id = str(uuid.uuid4())
    with get_cursor() as cur:
        cur.execute(
            """
            INSERT INTO runs (id, session_id, orchestrator, status, metadata)
            VALUES (%s, %s, %s, %s, %s)
            """,
            (run_id, session_id, orchestrator, "running", as_jsonb(metadata)),
        )
    return run_id
