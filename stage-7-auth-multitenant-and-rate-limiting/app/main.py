...
app = FastAPI(title="Creator Orchestrator API", version="0.7.0")
...
async def orchestrate(payload: OrchestrateRequest,
    ...
    # 1) Rate limit per tenant+user (raises 429 over the limit).
    enforce_rate_limit(tenant_id, user_id)

    # 2) Open a session + run (short-term + episodic memory), tagged by tenant.
    ...
    try:
        # 3) Time the run (observability) — the orchestrator does not own the
