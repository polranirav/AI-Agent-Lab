...
def create_session(user_id: Optional[str] = None,
    ...
    """Create a new session (tagged with its tenant) and return its UUID."""
    session_id = str(uuid.uuid4())
    with get_cursor(tenant_id=tenant_id) as cur:
        cur.execute(
            """
            INSERT INTO sessions (id, user_id, topic, metadata, tenant_id)
            VALUES (%s, %s, %s, %s, %s)
            """,
            (session_id, user_id, topic, as_jsonb(metadata), tenant_id),
        )
    return session_id
