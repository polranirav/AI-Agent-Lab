...
class BaseAgent:
    ...
    def run(self, user_message: str, context: str = "",
        ...
        Stage 4 additions:
          - Pre-LLM guardrails (PII redaction + injection detection) run on the
            user_message before the model sees it. A block short-circuits the run.
          - Post-LLM guardrails (brand/content policy) run on the final answer
            before it is returned. A block short-circuits; a modify rewrites it.
          - user_id flows into tool execution so RBAC can be enforced.
        """
        # If no session provided, create one so memory is always recorded.
        if session_id is None:
            session_id = short_term.create_session(topic=self.name)

        # ── Pre-LLM guardrails (input side) ───────────────────────────
        pre_decision = guardrails.apply_pre_llm_guards(
            user_message, run_id=run_id, session_id=session_id,
        )
        if not pre_decision.allowed:
            return f"Request blocked by guardrails: {pre_decision.reason}"
        # Use the sanitized (PII-redacted) text from here on.
        user_message = pre_decision.content or user_message
        ...
        for iteration in range(self.max_iterations):
            ...
            # Observability: accumulate token usage for this run.
            if response.usage and run_id is not None:
                add_token_usage(run_id,
            ...
            if message.tool_calls:
                ...
                # ── Post-LLM guardrails (output side) ─────────────────
                post_decision = guardrails.apply_post_llm_guards(
                    final, run_id=run_id, session_id=session_id,
                )
                if not post_decision.allowed:
                    short_term.append_message(
                        session_id, "assistant",
                        f"[BLOCKED] {post_decision.reason}",
                    )
                    if run_id is not None:
                        episodic.log_event(
                            run_id,
                            event_type="agent_end",
                            message=f"{self.name} output blocked by guardrails",
                            agent_name=self.name,
                            payload={"reason": post_decision.reason},
                        )
                    return f"Response blocked by guardrails: {post_decision.reason}"

                # Use the (possibly modified) post-guard content.
                final = post_decision.content or final
                ...
                if run_id is not None:
                    episodic.log_event(
                        ...
                        payload={"final_preview": final[:200],
                                 "guardrail_action": post_decision.action},
                    )
                return final
