...
## Plans, Quotas & Billing (Stage 8)

Every tenant has exactly one active **subscription** to a **plan** (`free`,
`pro`, `enterprise`). Each `/api/orchestrate` call:

1. checks the plan **quota** for the billing period — over quota returns **402**;
2. records one `VIDEO_INGESTED` usage event in `billing_usage` (what Stripe bills from);
3. reads per-tenant **feature flags** (e.g. `use_premium_model`) to configure behavior.

**Two limits, two status codes — don't confuse them:**

| Code | Meaning | Source |
|------|---------|--------|
| **429** | Too many requests *too fast* (velocity) | Stage 7 rate limiter |
| **402** | Plan quota for the period is used up (volume) | Stage 8 billing quota |

Plans (default seed): `free` = 5 videos/month, `pro` = 50, `enterprise` = custom.

Usage reporting to Stripe is a batch job over `billing_usage`
(`scripts/report_usage_to_stripe.py`, dry-run scaffold).
